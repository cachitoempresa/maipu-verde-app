...
import {
  ...
  SearchCode, HardHat, Scissors // Importado icono para Infra y Poda
} from 'lucide-react';
...
const usersList: LoginUser[] = [
  ...
  // PERFIL AGREGADO: Encargado Infraestructura
  { name: 'Encargado Infra', email: 'infra@maipu.cl', role: 'Encargado Infraestructura', type: 'infra' },
  { name: 'Cuadrilla Poda', email: 'poda@maipu.cl', role: 'Operativo Poda', type: 'poda' },
      ...
      <div className="w-full max-w-md p-6 relative z-10">
        ...
        <div className="bg-white/95 backdrop-blur-xl rounded-[2.5rem] shadow-2xl overflow-hidden border border-white/20 relative">
          <div className="px-8 py-5 border-b border-gray-100 bg-white flex items-center gap-3">
            {selectedUser ? (
              <button onClick={() => { setSelectedUser(null); setPassword(''); setError(''); }} className="text-slate-400 hover:text-slate-600 transition-all active:scale-90">
                    ...
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-bold text-sm shadow-sm transition-transform group-hover:scale-105
                          ...
                          user.type === 'its' ? 'bg-indigo-600 text-white' :
                            user.type === 'aljibe' ? 'bg-emerald-600 text-white' :
                              user.type === 'infra' ? 'bg-amber-500 text-slate-900' : // Color Ámbar para Infra
                                user.type === 'poda' ? 'bg-orange-600 text-white' :
                                  'bg-green-500 text-white'}`}>

                        {/* ASIGNACIÓN DE ICONOS MEJORADA */}
                        {user.type === 'aljibe' ? <Truck size={20} /> :
                          user.type === 'its' ? <SearchCode size={20} /> :
                            user.type === 'supervisor' ? <User size={20} /> :
                              user.type === 'infra' ? <HardHat size={20} /> : // Icono de Casco para Infra
                                user.type === 'poda' ? <Scissors size={20} /> :
                                  user.name.charAt(0)}
                      </div>
              ...
              <form onSubmit={handleLogin} className="p-6 animate-in fade-in slide-in-from-right-8">
                <div className="flex items-center gap-4 mb-8 bg-slate-50 p-4 rounded-3xl border border-slate-100">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-bold text-white shadow-md
                      ...
                      selectedUser.type === 'its' ? 'bg-indigo-600' :
                        selectedUser.type === 'infra' ? 'bg-amber-500 text-slate-900' :
                          selectedUser.type === 'poda' ? 'bg-orange-600' :
                            'bg-green-600'}`}>
                    {selectedUser.type === 'infra' ? <HardHat size={20} /> : selectedUser.type === 'poda' ? <Scissors size={20} /> : selectedUser.name.charAt(0)}
                  </div>
                  <div>
