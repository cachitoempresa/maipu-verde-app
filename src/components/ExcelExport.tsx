...
export function ExcelExport() {
  ...
  const handleExport = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase.from('logs').select('*, green_areas(code, name, current_status)').order('timestamp', { ascending: false });
      ...
      XLSX.writeFile(workbook, `Reporte_Maipu_NUBE_${new Date().toLocaleDateString().replace(/\//g, '-')}.xlsx`);
  ...
  return (
    <button onClick={handleExport} disabled={loading} className="w-full bg-green-600 text-white font-bold py-3 px-4 rounded-xl shadow-lg shadow-green-600/20 hover:bg-green-700 transition-all flex items-center justify-center gap-2 active:scale-95">
